...
function changeMe(arr) {
  if (!arr.length){
    ...
    for (let i = 0 ; i < arr.length ; i++){
      ...
      console.log(chnged.firstName ,chnged.lastName,":", chnged)
    } 
  }
...
function shoppingTime(memberId, money) {
  ...
  } else if (!memberId , !money || memberId === "") { 
    return "Daftar member dulu gih"
  } else {
    let stuff = {
...
function countProfit(shoppers) {
  ...
  for (let b = 0 ; b < listBarang.length ; b++){
    let stock = listBarang[b][2];
    let harga = listBarang[b][1];
    let stuffs = listBarang[b][0];
    ...
    for (let i = 0; i < shoppers.length; i++) {
      if (shoppers[i].product === stuffs && shoppers[i].amount <= obje.leftOver) {
        obje.shoppers.push(shoppers[i].name);
        obje.leftOver -= shoppers[i].amount;
        obje.totalProfit += shoppers[i].amount * harga;
      } 
